...
class StockTradingEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            # =====================
            # Calculate the amount of trading (portfolio turnover)
            trade_amount = np.sum(np.abs(actions))

            trade_penalty = - 0.1 * (1 - trade_amount / (self.hmax * self.stock_dim))

            # New reward: profit + activity incentive/penalty
            self.reward = (end_total_asset - begin_total_asset) * self.reward_scaling + trade_penalty
            # =====================
