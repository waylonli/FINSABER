...
class ExperimentRunner:
    ...
    def run(
        ...
        strat_config = (
            json.load(open(strat_config_path, encoding="utf-8"))
            if strat_config_path
            else None
        )
        tradingagents_run_identity = None
        if strat_config is not None:
            ...
            default_config, strat_config, tradingagents_run_identity = (
                self._materialize_tradingagents_run_root(
                    strategy_class=strategy_class,
                    trade_config=default_config,
                    strat_config=strat_config,
                )
            )
        if tradingagents_run_identity is not None:
            self._write_tradingagents_launcher_artifacts(
                run_identity=tradingagents_run_identity,
                setup_name=setup_name,
                strategy_class=strategy_class,
                trade_config=default_config,
