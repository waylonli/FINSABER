...
from llm_traders.finagent.query import DiverseQuery, extract_query_type
...
def prepared_tools_params(state: Dict,
    ...
    for i in range(4):
        strategy_number = i + 1
        # turn the index of dataframe price from datetime to datetime.date
        price.index = pd.to_datetime(price.index)
        price.index = price.index.date
        data = price[price.index <= pd.to_datetime(date).date()]
