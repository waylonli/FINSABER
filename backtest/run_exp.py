...
def run_cherry_exps(mode:str="both_finmem"):
    if mode == "both_finmem" or mode == "cherry_pick_both_finmem":
        mode = "cherry_pick_both_finmem"
        trade_config = {
    ...
    elif mode == "both_fincon" or mode == "cherry_pick_both_fincon":
        mode = "cherry_pick_both_fincon"
        trade_config = {
    ...
    for _, strategy in inspect.getmembers(strat_namespace):
        ...
        if os.path.exists(os.path.join("preliminary", "output", mode, f"{strategy.__name__}")):
            print(f"Output file for {strategy.__name__} exists, skipping...")
            continue
...
def run_main_exps(mode:str="selected_5"):
    if mode == "selected_5":
        trade_config = {
            "tickers": ["TSLA", "NFLX", "AMZN", "MSFT", "COIN"],
            "silence": True,
        ...
        trade_config = {
            "tickers": "all",
            "silence": True,
            "selection_strategy": f"random:{mode.split('_')[1]}",
        }
    else:
        raise ValueError(f"Invalid mode: {mode}")
    ...
    for _, strategy in inspect.getmembers(strat_namespace):
        ...
        try:
            operator = BacktestingEngine(trade_config)
            metric = operator.run_rolling_window(strategy)
            eval_metrics.append(metric)
        except Exception as e:
            print(f"Error running strategy {strategy.__name__}: {e}")
            continue
    ...
    print("Finish backtesting all strategies, aggregating results...")
    aggregate_results(trade_config["selection_strategy"].replace(":", "_"))
